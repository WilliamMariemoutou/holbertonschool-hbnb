...
async function fetchPlaces(token) {
    try {
        ...
        setupPriceFilter(); // Setup filter after places are loaded
...
function setupPriceFilter() {
    ...
    if (!priceFilter || placeCards.length === 0) return;

    // Populate dropdown dynamically based on prices present
    const prices = [...new Set(Array.from(placeCards).map(c => Number(c.dataset.price)))].sort((a, b) => a - b);
